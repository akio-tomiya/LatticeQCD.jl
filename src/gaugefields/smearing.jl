            ...
            for iz = 1:NZ
                for iy = 1:NY
                    for ix = 1:NX
                        ...
                        #Q = (im/2)*(Ω' .- Ω) .- (im/(2NC))*tr(Ω' .- Ω)
                        Uout[μ][:, :, ix, iy, iz, it] =
                            K * inv(A) * besseli(2, β * K) / besseli(1, β * K)
...
function calc_fatlink_APE!(
    Uout::Array{GaugeFields{SU{NC}},1},
    α,
    β;
    normalize_method = "special unitary",
    temporal_dir_smear = true,
) where {NC}
    Uin = deepcopy(Uout)
    calc_fatlink_APE!(
        Uout,
        ...
        temporal_dir_smear = temporal_dir_smear,
    )
end
...
function calc_fatlink_APE!(
    ...
    nmethod(A) =
        ...
        elseif normalize_method == "special unitary"
            ...
            error(
                "normalize_method should be unitary or special unitary. Now, $normalize_method",
            )
        end
