...
function evaluate_loops!(
    ...
    loops::Loops,
    U::Array{GaugeFields{S},1},
) where {S<:SUn,T_1d<:GaugeFields_1d}
...
function calc_coordinate_shift(loopk, coordinates)
    ...
    elseif loopk[2] == -1
        if loopk[1] == 1
            shifts =
                (coordinates[1] + loopk[2], coordinates[2], coordinates[3], coordinates[4])
        elseif loopk[1] == 2
            shifts =
                (coordinates[1], coordinates[2] + loopk[2], coordinates[3], coordinates[4])
        elseif loopk[1] == 3
            shifts =
                (coordinates[1], coordinates[2], coordinates[3] + loopk[2], coordinates[4])
        elseif loopk[1] == 4
            shifts =
                (coordinates[1], coordinates[2], coordinates[3], coordinates[4] + loopk[2])
        end
    end
...
function LinearAlgebra.mul!(
    c::GaugeFields_1d{SU{NC}},
    a::GaugeFields_1d{SU{NC}},
    b::GaugeFields_1d{SU{NC}},
) where {NC}
...
function LinearAlgebra.mul!(
    ...
    @simd for i = 1:NV
        c[1, 1, i] =
            a[1, 1, i] * b[1, 1, i] + a[1, 2, i] * b[2, 1, i] + a[1, 3, i] * b[3, 1, i]
        c[1, 2, i] =
            a[1, 1, i] * b[1, 2, i] + a[1, 2, i] * b[2, 2, i] + a[1, 3, i] * b[3, 2, i]
        c[1, 3, i] =
            a[1, 1, i] * b[1, 3, i] + a[1, 2, i] * b[2, 3, i] + a[1, 3, i] * b[3, 3, i]

        c[2, 1, i] =
            a[2, 1, i] * b[1, 1, i] + a[2, 2, i] * b[2, 1, i] + a[2, 3, i] * b[3, 1, i]
        c[2, 2, i] =
            a[2, 1, i] * b[1, 2, i] + a[2, 2, i] * b[2, 2, i] + a[2, 3, i] * b[3, 2, i]
        c[2, 3, i] =
            a[2, 1, i] * b[1, 3, i] + a[2, 2, i] * b[2, 3, i] + a[2, 3, i] * b[3, 3, i]

        c[3, 1, i] =
            a[3, 1, i] * b[1, 1, i] + a[3, 2, i] * b[2, 1, i] + a[3, 3, i] * b[3, 1, i]
        c[3, 2, i] =
            a[3, 1, i] * b[1, 2, i] + a[3, 2, i] * b[2, 2, i] + a[3, 3, i] * b[3, 2, i]
        c[3, 3, i] =
            a[3, 1, i] * b[1, 3, i] + a[3, 2, i] * b[2, 3, i] + a[3, 3, i] * b[3, 3, i]
    end
...
function LinearAlgebra.mul!(
    c::GaugeFields_1d{SU{NC}},
    a::GaugeFields_1d{SU{NC}},
    b::Adjoint_GaugeFields_1d{SU{NC}},
) where {NC}
...
function LinearAlgebra.mul!(
    ...
    @simd for i = 1:NV
        c[1, 1, i] =
            a[1, 1, i] * conj(b.parent[1, 1, i]) + a[1, 2, i] * conj(b.parent[1, 2, i])
        c[1, 2, i] =
            a[1, 1, i] * conj(b.parent[2, 1, i]) + a[1, 2, i] * conj(b.parent[2, 2, i])

        c[2, 1, i] =
            a[2, 1, i] * conj(b.parent[1, 1, i]) + a[2, 2, i] * conj(b.parent[1, 2, i])
        c[2, 2, i] =
            a[2, 1, i] * conj(b.parent[2, 1, i]) + a[2, 2, i] * conj(b.parent[2, 2, i])
...
function LinearAlgebra.mul!(
    ...
    @simd for i = 1:NV
        c[1, 1, i] =
            conj(a.parent[1, 1, i]) * conj(b.parent[1, 1, i]) +
            conj(a.parent[2, 1, i]) * conj(b.parent[1, 2, i])
        c[1, 2, i] =
            conj(a.parent[1, 1, i]) * conj(b.parent[2, 1, i]) +
            conj(a.parent[2, 1, i]) * conj(b.parent[2, 2, i])

        c[2, 1, i] =
            conj(a.parent[1, 2, i]) * conj(b.parent[1, 1, i]) +
            conj(a.parent[2, 2, i]) * conj(b.parent[1, 2, i])
        c[2, 2, i] =
            conj(a.parent[1, 2, i]) * conj(b.parent[2, 1, i]) +
...
function LinearAlgebra.mul!(
    ...
    @simd for i = 1:NV
        ...
        c[1, 3, i] =
            conj(a.parent[1, 1, i]) * conj(b.parent[3, 1, i]) +
...
function LinearAlgebra.mul!(
    c::GaugeFields_1d{SU{NC}},
    a::Adjoint_GaugeFields_1d{SU{NC}},
    b::Adjoint_GaugeFields_1d{SU{NC}},
) where {NC}
...
function LinearAlgebra.mul!(
    c::GaugeFields_1d{SU{NC}},
    Udag::Adjoint_GaugeFields{SU{NC}},
) where {NC} #Udag*c
    NV = c.NV
    NT = Udag.parent.NT
    NZ = Udag.parent.NZ
                ...
                for ix = 1:NX
                    ...
                    for k2 = 1:NC
                        for k1 = 1:NC
                            ...
                            @simd for k3 = 1:NC
                                tmp[k1, k2] +=
                                    conj(Udag.parent[k3, k1, ix, iy, iz, it]) * c[k3, k2, i]
                            end
                        end
                    end
                    @. c[:, :, i] = tmp[:, :]
                ...
                for ix = 1:NX
                    ...
                    for k2 = 1:NC
                        for k1 = 1:NC
                            ...
                            @simd for k3 = 1:NC
                                c[k1, k2, i] +=
                                    conj(a.parent[k3, k1, ix, iy, iz, it]) * b[k3, k2, i]
                            end
                        end
                    end
                end
...
function LinearAlgebra.mul!(
    ...
    @simd for i = 1:NV
        ...
        c[1, 1, i] =
            conj(a.parent[1, 1, i]) * b[1, 1, i] + conj(a.parent[2, 1, i]) * b[2, 1, i]
        c[1, 2, i] =
            conj(a.parent[1, 1, i]) * b[1, 2, i] + conj(a.parent[2, 1, i]) * b[2, 2, i]


        c[2, 1, i] =
            conj(a.parent[1, 2, i]) * b[1, 1, i] + conj(a.parent[2, 2, i]) * b[2, 1, i]
        c[2, 2, i] =
            conj(a.parent[1, 2, i]) * b[1, 2, i] + conj(a.parent[2, 2, i]) * b[2, 2, i]
...
function periodiccheck(ix, iy, iz, it, NX, NY, NZ, NT, NDW)
    return periodiccheck(ix, NX, NDW),
    periodiccheck(iy, NY, NDW),
    periodiccheck(iz, NZ, NDW),
    periodiccheck(it, NT, NDW)
end
...
function lambdamul(
    b::SUNGaugeFields_1d{NC},
    a::SUNGaugeFields_1d{NC},
    k,
    generator,
) where {NC}
    #=
...
function calc_Plaq!(
    U::Array{T,1},
    temps::Array{T_1d,1},
) where {T<:GaugeFields,T_1d<:GaugeFields_1d}
    plaq = 0
    temp1 = temps[1]
    temp2 = temps[2]
    temp3 = temps[3]
...
function calc_Plaq!(
    U::Array{T,1},
    temps::Array{T_1d,1},
) where {T<:GaugeFields,T_1d<:GaugeFields}
    plaq = 0
    temp1 = temps[1]
    temp2 = temps[2]
    temp3 = temps[3]
...
function evaluate_tensor_matrix!(
    temp2,
    temp3,
    U,
    tensorlist,
...
function evaluate_tensor!(
    leftmatrix,
    rightmatrix,
    tensor::Tensor_wilson_lines,
    U::Array{GaugeFields{SU{NC}},1},
    ix,
    iy,
    iz,
            ...
            for j = 1:NC
                for i = 1:NC
                    ...
                    for k = 1:NC
                        M[i, j] +=
                            trB2 * Q[i, k] * Q[k, j] +
                            f2 * (Q[i, k] * UdSdU[k, j] + UdSdU[i, k] * Q[k, j])
                    end
                end
            end
            M ./= im
...
function evaluate_tensor_lines(
    mu,
    dSdU::Array{T_1d,1},
    smearing,
    U::Array{GaugeFields{SU{NC}},1},
    ...
    it,
    ρs,
) where {NC,I<:Int,T_1d<:GaugeFields_1d}
            ...
            for (k, timu_k) in enumerate(timu)
                if haskey(timu_k, umu)
                    ...
                    for m = 1:length(timuumu)
                        ...
                        ix1, iy1, iz1, it1 = calc_matrix_tensors!(
                            leftmatrix,
                            rightmatrix,
                            tensor,
                            U,
                        ...
                        for i = 1:NC
                            for j = 1:NC
                                tmp_matrices[3][j, i] =
                                    conj(U[nu][i, j, ix1, iy1, iz1, it1]) #U[nu][:,:,ix1,iy1,iz1,it1]'
                            end
                        end
...
function calc_smearingU(
    Uin::Array{GaugeFields{SU{NC}},1},
    smearing;
    calcdSdU = false,
    temps = nothing,
) where {NC}
...
function apply_smearing(
    U::Array{GaugeFields{SU{NC}},1},
    smearing::T,
) where {NC,T<:SmearingParam_single}
    Uout = similar(U)
...
function calc_stout!(
    Uout::Array{GaugeFields{SU{NC}},1},
    U::Array{GaugeFields{SU{NC}},1},
    ρ,
) where {NC}
...
function calc_stout_multi!(
    Uout_multi::Array{Array{GaugeFields{SU{NC}},1},1},
    Uin::Array{GaugeFields{SU{NC}},1},
    ρs::Array{Array{T,1},1},
    staples,
) where {NC,T<:Number}
