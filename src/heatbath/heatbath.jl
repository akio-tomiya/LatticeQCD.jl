...
import ..Wilsonloops:
    Wilson_loop, Wilson_loop_set, make_plaq_staple, make_links, make_staples, make_plaq
import ..Actions:
    GaugeActionParam_standard, GaugeActionParam_autogenerator, GaugeActionParam
...
function heatbath!(
    ...
    temps::Array{T_1d,1},
) where {T<:SU2GaugeFields,T_1d<:SU2GaugeFields_1d}
    beta = gparam.β
            ...
            for iz = 1:NZ
                for iy = 1:NY
                    for ix = 1:NX
                        ...
                        u[mu][:, :, ix, iy, iz, it] =
                            SU2update_KP(V, beta, NC, ITERATION_MAX)
...
function SU2update_KP(V, beta, NC, ITERATION_MAX = 10^5)
    ...
    a[4] = rr * cosθ
    Unew = [
        a[1]+im*a[4] a[3]+im*a[2]
        -a[3]+im*a[2] a[1]-im*a[4]
    ] * V0
...
function heatbath!(
    ...
    gparam,
    temps::Array{T_1d,1},
) where {T<:SU3GaugeFields,T_1d<:SU3GaugeFields_1d}
    #println("Warning!!!!!!!!")
...
function heatbath!(
    ...
    gparam,
    temps::Array{T_1d,1},
) where {T<:SUNGaugeFields,T_1d<:SUNGaugeFields_1d}
    #println("Warning!!!!!!!!")
    #error("Heatbath update for SU(3) is not implemented")
    beta = gparam.β
