...
function trlog(A, itemax, num, eps)
    println(
        "Estimation of log(det(A))=Tr(log(A)) start------------------------------------------",
    )
    ...
    for i = 1:num
        ...
        if i > 1
            ...
            if abs(oldvalue - value / i) < eps2
                println(
                    "In the $i-th step, this is converged. Absolute error might be ",
                    abs(oldvalue - value / i),
                )
                println(
                    "Estimation of log(det(A))=Tr(log(A)) end------------------------------------------",
                )
                return value / i
            end
