...
mutable struct Chiral_condensate_measurement{Dim,TG,TD,TF,TF_vec} <: AbstractMeasurement
    ...
    function Chiral_condensate_measurement(U::Vector{T};
                                        filename = nothing,
                                        verbose_level = 2,
                                        printvalues = true,
        ...
        elseif fermiontype == "Domainwall"
            ...
            error("fermion type $fermiontype is not supported in chiral condensate measurement")
        end          
        ...
        return new{Dim,T,TD,TF,TF_vec}(filename,_temporary_gaugefields,Dim,verbose_print,printvalues,
                            D,#::TD
                            fermi_action,#::TF,
...
function measure(m::M,itrj,U::Array{<: AbstractGaugefields{NC,Dim},1};additional_string = "") where {M <: Chiral_condensate_measurement,NC,Dim}
    ...
    for ir=1:Nr
        ...
        if m.printvalues
            ...
            println_verbose_level2(m.verbose_print,"# $itrj $ir $additional_string $(real(tmp)/U[1].NV) # itrj irand chiralcond")
        end
        pbp+=tmp
    end
