module Clover
    ...
    import ..Gaugefield:Wilson_loop,Wilson_loop_set,AbstractGaugefields,set_wing_U!,
                Loops,evaluate_loops!
    import ..FermionAction_module:FermiActionParam_WilsonClover

    function Make_CloverFμν(fparam::FermiActionParam_WilsonClover,U,temps::Array{T,1})  where T <: AbstractGaugefields
        NV = temps[1].NV
        NC = temps[1].NC
    ...
    function Make_CloverFμν!(CloverFμν,fparam::FermiActionParam_WilsonClover,U,temps::Array{<: AbstractGaugefields{NC,Dim},1}) where {NC,Dim}
        @assert Dim==4 "Now only Dim = 4 is supported in WilsonClover action"
        work1 = temps[1]
        work2 = temps[2]
        work3 = temps[3]
        work4 = temps[4]
    ...
    function setFμν!(Fμν,μν,v::AbstractGaugefields{NC,Dim}) where {NC,Dim}
        NV=v.NV
        for i=1:NV
            for k2=1:NC
                for k1=1:NC
    ...
    function addFμν!(Fμν,μν,v::AbstractGaugefields{3,Dim}) where {Dim}
        NV=v.NV
    ...
    function addFμν!(Fμν,μν,v::AbstractGaugefields{2,Dim}) where {Dim}
        NV=v.NV
    ...
    function addFμν!(Fμν,μν,v::AbstractGaugefields{NC,Dim}) where {NC,Dim}
        NV=v.NV
    ...
    """
    function dSclover!(z,μ,X,Y,U,fparam::FermiActionParam_WilsonClover,temps::Array{T,1}) where T <: AbstractGaugefields
        NX = X.NX
        NY = X.NY
        NZ = X.NZ
        NT = X.NT
    ...
    function cal_dFμν!(dFμν1,dFμν2,U,fparam::FermiActionParam_WilsonClover,work1::T,work2,work3,work4,temp1,temp2,temp3,temp4,μ,ν,iflag) where T <: AbstractGaugefields
        coe = im*0.125*fparam.hop*fparam.Clover_coefficient
        NC = U[1].NC
        #numbasis = ifelse(NC == 3,8,3)
        numbasis = NC^2-1
    ...
    """
        x <= x - x_aj
    """
    function cimaglnk!(x::AbstractGaugefields{NC,Dim}) where {NC,Dim}
        NV = x.NV
    ...
    function cimaglnk!(x::AbstractGaugefields{3,Dim}) where Dim
        NV = x.NV
        for i=1:NV
    ...
    function cimaglnk!(x::AbstractGaugefields{2,Dim}) where Dim
        NV = x.NV
        for i=1:NV
            z11 = x[1,1,i] - conj(x[1,1,i]) 
            z12 = x[1,2,i] - conj(x[2,1,i]) 
