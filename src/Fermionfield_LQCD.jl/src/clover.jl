module Clover
    ...
    import ..Gaugefields:GaugeFields_1d,GaugeFields,set_wing!,
                ...
                =#
    import ..Gaugefield:Wilson_loop,Wilson_loop_set,AbstractGaugefields
    ...
    function Make_CloverFμν!(CloverFμν,fparam::FermiActionParam_WilsonClover,U,temps::Array{T,1}) where T <: AbstractGaugefields
        work1 = temps[1]
        work2 = temps[2]
        work3 = temps[3]
        work4 = temps[4]
    ...
    function setFμν!(Fμν,μν,v::AbstractGaugefields{Dim,NC}) where {NC,Dim}
        NV=v.NV
        for i=1:NV
            for k2=1:NC
                for k1=1:NC
    ...
    function addFμν!(Fμν,μν,v::AbstractGaugefields{Dim,3}) where {Dim}
        NV=v.NV
    ...
    function addFμν!(Fμν,μν,v::AbstractGaugefields{Dim,2}) where {Dim}
        NV=v.NV
    ...
    function addFμν!(Fμν,μν,v::AbstractGaugefields{Dim,NC}) where {Dim,NC}
        NV=v.NV
    ...
    """
        x <= x - x_aj
    """
    function cimaglnk!(x::AbstractGaugefields{Dim,NC}) where {Dim,NC}
        NV = x.NV
        for i=1:NV
            for k2=1:NC
                for k1=k2:NC
    ...
    function cimaglnk!(x::AbstractGaugefields{Dim,3}) where {Dim}
        NV = x.NV
        for i=1:NV
    ...
    function cimaglnk!(x::AbstractGaugefields{Dim,2}) where {Dim}
        NV = x.NV
        for i=1:NV
            z11 = x[1,1,i] - conj(x[1,1,i]) 
            z12 = x[1,2,i] - conj(x[2,1,i]) 
